...
def burlirsch_stoer_method (satellite, time, A, N, Y, m, adapted_thrust=False) :
	...
	for j in range (cst.jmax+1) :

		A[j][0] = satellite.pas_pointmilieu_modifie(cst.H, time, Y, m[j], adapted_thrust)
		...
		for k in range (N) : 
			e+=(abs(A[j][j][k]-A[j][j-1][k])/(cst.atol+cst.rtol*abs(A[j][j][k])))**2
		e = np.sqrt(e/N)
		if(e<1):
			break
